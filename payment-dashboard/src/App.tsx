...
function Dashboard() {
  ...
  const { selectedIds, toggleSelection, selectAll, clearSelection } = useStore();
  ...
  const getStatusColor = (status: Status) => {
    switch(status) {
      ...
      case 'Due': return 'bg-red-100 text-red-700 hover:bg-red-100 border-red-200';
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            ...
            {selectedIds.length === 1 ? (
              ...
              <Button onClick={() => { clearSelection(); setIsModalOpen(true); }}>
          ...
          <Table>
              ...
              <TableRow>
                <TableHead className="w-[50px]">
                   <Checkbox 
                    ...
                    onCheckedChange={(checked) => checked ? selectAll(customers.map(c => c.id)) : clearSelection()}
                   />
                </TableHead>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow><TableCell colSpan={7} className="h-24 text-center text-slate-500">No Data Found</TableCell></TableRow>
              ) : (
                customers.map((customer) => (
                  <TableRow key={customer.id} data-state={selectedIds.includes(customer.id) && "selected"}>
                    <TableCell>
                      <Checkbox 
                    ...
                    <TableCell>
                      <Badge className={`font-normal border ${getStatusColor(customer.status)}`}>
                        {customer.status}
                      </Badge>
                    </TableCell>
                    <TableCell className="hidden md:table-cell text-slate-500">{customer.description}</TableCell>
                    <TableCell className="text-right font-mono">${customer.rate.toFixed(2)}</TableCell>
                    <TableCell className="text-right font-mono text-slate-500">${customer.deposit.toFixed(2)}</TableCell>
                    <TableCell className="text-right font-mono font-medium">${customer.balance.toFixed(2)}</TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
